...
- Publishing a GitHub Release also triggers `deploy-azure-vm.yml`: auto-detects VM in Terraform state and runs `full` (provision + deploy) or `deploy` (Ansible only)
